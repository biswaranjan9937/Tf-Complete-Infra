...
resource "aws_subnet" "public" {
  ...
  cidr_block = var.public_subnet_ipv6_native ? null : (count.index < length(var.public_subnets) ? var.public_subnets[count.index] : null)
  ...
  tags = merge(
    {
      Name = try(
        ...
        upper(format("${var.name}-%s-INFRA-${var.public_subnet_suffix}-SUBNET", element(var.azs, count.index)))
      )
    },
    var.tags,
  ...
  tags = merge(
    {
      "Name" = var.create_multiple_public_route_tables ? format(
        "${var.name}-${var.public_subnet_suffix}-RT",
...
resource "aws_network_acl" "public" {
  ...
  tags = merge(
    { "Name" = "${var.name}-${var.public_subnet_suffix}-ACL" },
    var.tags,
    var.public_acl_tags,
  )
}
...
resource "aws_subnet" "private" {
  ...
  cidr_block                                     = var.private_subnet_ipv6_native ? null : (count.index < length(var.private_subnets) ? var.private_subnets[count.index] : null)
  ...
  tags = merge(
    {
      Name = try(
        ...
        upper(format("${var.name}-%s-APP-${var.private_subnet_suffix}-SUBNET", element(var.azs, count.index)))
      )
    },
    var.tags,
...
resource "aws_network_acl" "private" {
  ...
  tags = merge(
    { "Name" = "${var.name}-${var.private_subnet_suffix}-ACL" },
    var.tags,
  ...
  tags = merge(
    {
      Name = try(
        ...
        upper(format("${var.name}-%s-DB-${var.database_subnet_suffix}-SUBNET", element(var.azs, count.index)))
      )
    },
    var.tags,
  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway || var.create_database_internet_gateway_route ? "${var.name}-${var.database_subnet_suffix}" : format(
        "${var.name}-${var.database_subnet_suffix}-RT",
        element(var.azs, count.index),
      )
    },
    var.tags,
...
resource "aws_network_acl" "database" {
  ...
  tags = merge(
    { "Name" = "${var.name}-${var.database_subnet_suffix}-ACL" },
    var.tags,
